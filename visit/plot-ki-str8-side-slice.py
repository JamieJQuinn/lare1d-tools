...
for filename in filenames:
  for variable in variables:
    ...
    print filename
    visc_exp, resist_exp, visc_mode, extra = re.search('v-(\d)r-(\d)-(switching|isotropic)(.*?)/', filename).groups()
    t_step = re.search('/(\d+).sdf', filename).groups()[0]

    # Form output filenames
    if visc_mode == "switching":
      fig_fname = variable.split("/")[-1] + "_v-" + visc_exp + "r-" + resist_exp + "_switching_" + extra + "_" + t_step
    else:
      fig_fname = variable.split("/")[-1] + "_v-" + visc_exp + "r-" + resist_exp + "_isotropic_" + extra + "_" + t_step
